...
<div class="breadcrumb">
    ...
    <a href="{{ route('notifications_settings') }}" class="go-back">
        {{ __('Notificações') }}</a>
    <span>\</span>
    <span>{{ __('Editar Notificação')}}</span>
</div>
<section class="main-content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-xs-12">
                <!-- page title -->
                <h2 class="page-title">{{ __('Editar Notificação')}}</h2>
                <!-- page title ends -->
                <!-- form to add banner -->
                <form action="{{ route('notification_update',['id' => $notifications->id]) }}" method="post" class="row form" enctype="multipart/form-data">
                    @csrf
